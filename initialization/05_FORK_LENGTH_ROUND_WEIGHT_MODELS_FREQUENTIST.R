...
LM_IO_BET_FORK_LENGTH_ROUND_WEIGHT = lm(log(whole_fish_weight) ~ log(fork_length), data = IO_BET_FORK_LENGTH_ROUND_WEIGHT_DATASET)

a_LM_FL_RW_BET_IO <- exp(coefficients(LM_IO_BET_FORK_LENGTH_ROUND_WEIGHT)[1]) * exp(var(residuals(LM_IO_BET_FORK_LENGTH_ROUND_WEIGHT))/2)

b_LM_FL_RW_BET_IO = coefficients(LM_IO_BET_FORK_LENGTH_ROUND_WEIGHT)[2]

IO_BET_FORK_LENGTH_ROUND_WEIGHT_DATASET[, LOG_ROUND_WEIGHT := log(whole_fish_weight)]
...
IO_BET_FORK_LENGTH_ROUND_WEIGHT_DATASET[, ROUND_WEIGHT_PREDICTED := exp(LOG_ROUND_WEIGHT_PREDICTED)]
IO_BET_FORK_LENGTH_ROUND_WEIGHT_DATASET[, ROUND_WEIGHT_PREDICTED_BIAS :=  a_LM_FL_RW_BET_IO*fork_length^b_LM_FL_RW_BET_IO]

# Data frame for predictions
IO_FL_BET_FOR_PREDICTION = data.table(fork_length = seq(28, 174, 0.1))
IO_FL_BET_FOR_PREDICTION[, whole_fish_weight_predicted := a_LM_FL_RW_BET_IO*fork_length^b_LM_FL_RW_BET_IO]
...
ggplot(BET_FORK_LENGTH_ROUND_WEIGHT_DATASET, aes(x = fork_length, y = whole_fish_weight)) +
  ...
  theme_bw() +
  geom_line(data = IO_FL_BET_FOR_PREDICTION, aes(x = fork_length, y = whole_fish_weight_predicted), size = 1.2, color = "#0073C299") +
