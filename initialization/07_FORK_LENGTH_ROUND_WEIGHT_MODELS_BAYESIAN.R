...
nls2_jags <- function(){
  ...
  a       = 10^(log10a) * 10^(sigma^2)/2    # a with bias correction
...
FL_RW_MODEL = jags(data = JAGS_DATA, inits = init_values, parameters.to.save = params, model.file = nls2_jags, n.chains = 3, n.iter = 50000, n.burnin = 2000, n.thin = 10, DIC = T)
...
mean_predictions <- exp(as.numeric(fit_nls2$BUGSoutput$mean$alpha) + as.numeric(fit_nls2$BUGSoutput$mean$beta) * JAGS_DATA$logfl)

### Array of predictions
predictions <- array(dim = c(JAGS_DATA$N, fit_nls2$BUGSoutput$n.sims))

for (i in 1:JAGS_DATA$N){
  predictions[i,] <-  exp(fit_nls2$BUGSoutput$sims.list$alpha + fit_nls2$BUGSoutput$sims.list$beta * JAGS_DATA$logfl[i])
}

# Lower bound
LPB <- apply(predictions, 1, quantile, 0.025, na.rm = TRUE)   #2.5% quantile for each observation logFL
UPB <- apply(predictions, 1, quantile, 0.975, na.rm = TRUE)  #97.5% quantile for each observation logFL

### Plot the observations
plot(exp(JAGS_DATA$logfl), exp(JAGS_DATA$logwt), cex.axis = 1.3, cex.lab = 1.3, las = 1, xlab = expression(paste("Fork length ",italic(L[F])," (cm)",sep="")), ylab = expression(paste("Round weight ", italic(W[T])," (kg)",sep="")), pch = 19, col = "darkgrey", cex = 0.3)

### Add the mean predictions
lines(sort(exp(JAGS_DATA$logfl)), sort(mean_predictions),col="black",lwd=2)

### Add the credible interval (uncertainty interval from the actual data)
lines(sort(exp(JAGS_DATA$logfl)), sort(LPB), col = "red", lty = 2)
lines(sort(exp(JAGS_DATA$logfl)), sort(UPB), col = "red", lty = 2)
